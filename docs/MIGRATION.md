...
| `src/pages/Portfolio.tsx` | 🏗️ REBUILD | 5.5 | Reescribir galería con `react-compare-slider` (cierra DT-12), 4 bloques nuevos (ver CONTENT.md §4). Elimina toggle manual de opacidad. Elimina fondo gradiente naranja-rojo. |
...
| Path | Clasificación | Fase | Notas |
|---|---|---|---|
| `src/components/GallerySection.tsx` | 🗑️ DESCARTAR si existe como parte de Portfolio actual | 5.5 | Si el archivo existe y se usaba en Portfolio.tsx, queda sin uso al reescribir la galería con `react-compare-slider` directamente en el nuevo `Portfolio.tsx`. Si se usa en otra parte (ej. dashboard), PRESERVAR. Validar en Fase 2 con depcheck. Cierra DT-10 parcial (console.error ocurrencias). |
...
| Componente nuevo: `PortfolioGallery.tsx` | 🆕 CREAR | 5.5 | Bloque C: grid 2 cols con `react-compare-slider` por item. Recibe array de items desde `src/data/galleryData.ts` (🆕 CREAR). |
...
| `src/components/portfolio/PortfolioGallery.tsx` | 5.5 | Galería con `react-compare-slider` |
