...
| Versión | 2.2 (May 26, 2026) — cierre Subfase 5.5a /portfolio REBUILD, D40-1 a D40-6, 4 componentes en `src/components/portfolio/`, DT-12 cerrado (slider con `react-compare-slider v4`), DT-17 cerrado (namespace pricing.* eliminado), drift librería slider resuelto |
...
│   │   ├── home/            ← secciones Home rebuild Subfase 5.1: Hero, HomeServices, HomeCasesWeb, HomePhotoShowcase, HomeProcess, HomeAbout, Testimonials
│   │   ├── layout/          ← chrome global canónico (D38-1, creada Fase 3 D35, formalizada 5.2): Header, Footer, MobileNav, LanguageToggle, WhatsAppButton
│   │   └── [custom directos: ScrollToTop, PageLoader]
│   ├── pages/
│   │   ├── Index.tsx (home)
...
├── package.json
└── components.json
> **Nota D38-1 (2026-05-12):** versiones anteriores de §5.2 prescribían "mantener estructura plana actual, no reorganizar en Sprint 1". En Fase 3 sub-tarea 3 (D35) se creó `src/components/layout/` como subcarpeta del chrome global. Validada G3 final 20/20 y G5 parcial 5.2. Formalizada como canónica a partir de v2.0. Análogo a D36-2 (rutas /servicios descartadas como dedicadas) y D35-5 (legal routes planas).
...
|---|---|---|---|---|
| `/` | `Index` | 1 | Sí | Home como single-page narrativa con 8 secciones (hero, services, cases-web, cases-photo, process, about, testimonials, contact) accesibles por anchors |
| `/portfolio` | `Portfolio` | 1 | Sí | Portfolio fotográfico, 4 bloques editoriales |
| `/portfolio-webs` | `PortfolioWebs` | 1 | Sí | Portfolio desarrollo web, 4 bloques editoriales |
| `/privacidad` | `PrivacyPolicy` | 1 | Sí | RGPD |
| `/cookies` | `CookiesPolicy` | 1 | Sí | Política cookies |
| `/aviso-legal` | `LegalNotice` | 1 | Sí | Aviso legal |
| `/styleguide` | `Styleguide` | 1 | Solo dev | Validación DS, excluida del bundle prod |
| `/auth` | `Auth` | 2 | Sí | Login / registro (ya existe, intocable Sprint 1) |
| `/dashboard` | `Dashboard` | 2 | Auth | Hub cliente (intocable Sprint 1) |
| `*` | `NotFound` | 1 | Sí | 404 rediseñada (Fase 4) |

**Modelo anchors-in-Home (D36-2, 2026-05-03):** las rutas `/servicios`, `/sobre`, `/contacto` que versiones previas de MASTER listaban como dedicadas quedan descartadas. Estas áreas viven como secciones del Home con anchors `/#services`, `/#about`, `/#contact`. El Header navega por anchors. ScrollToTop.tsx escucha cambios de hash y dispara scroll suave (D36-4).
...
**Sub-rutas dashboard Sprint 2:** versiones previas planteaban `/dashboard/paquetes`, `/dashboard/subir`, `/dashboard/pedidos`, `/dashboard/pedidos/:id`. La realidad actual del repo implementa Dashboard monolítico con tabs internas. Decisión sobre sub-rutas vs tabs queda diferida a Sprint 2 cuando se rediseñe el dashboard.
...
- `PortfolioGallery.tsx` (galería con `react-compare-slider`, cierra DT-12)
...
|---|---|---|---|---|
| DT-01 | `"34667326300"` hardcoded en 7 archivos (ContactForm, CTASection, Footer, FormSection, Header, PricingSection, Portfolio) + bug `34634408043` en WhyUs. Extraer a `VITE_WHATSAPP_NUMBER` | INVENTORY §16.2, AGENT.md §⚙️ | Fase 5.1-5.4 (REBUILD natural de cada componente: Header, Footer, Hero, FormSection, WhatsAppButton; CTASection, PricingSection, WhyUs DESCARTAR) | ✅ Resuelto 7/7 en Subfase 5.4, commits `92f7cf2` (descarte ContactForm + FormSection legacy en mismo commit que creación HomeContact), `5605764`, `3450fb5`. Grep final `git grep -nE "34667326300\|34634408043" -- src/` vacío. Nota retroactiva: conteo histórico "5/7 cerrados en 5.2" se mantiene en logs; aquí se firma cierre total |
| DT-02 | 14+ `any` explícitos en AuthContext, ContactForm, dashboard/*, processData.ts, useSecureNavigation | INVENTORY §16.5, MASTER §10.3 | Sprint 1: ContactForm.tsx en Fase 5.4 (REBUILD), processData.ts DESCARTAR; Sprint 2: AuthContext, dashboard, useSecureNavigation | Parcialmente resuelto Sprint 1. ContactForm.tsx cerrado por descarte en Subfase 5.4 (commit `92f7cf2`). processData.ts cerrado por descarte en Subfase 5.1. Pendiente Sprint 2: AuthContext, dashboard/, useSecureNavigation |
...
| DT-09 | `getStoredLanguage()` en `src/i18n/config.ts` siempre retorna `"es"`. Switch a EN no funcional | INVENTORY §16.17, MASTER §2.3 | Fase 3 sub-tarea 3 — sustituido por `i18next-browser-languagedetector` en D35-3 | ✅ Resuelto en Fase 3, commit `73535f7`. Confirmado en G5 parcial 5.2 (toggle ES↔EN funcional). Drift documental: §7.4 nunca se actualizó al cierre Fase 3 |
| DT-10 | `console.error()` activo en `GallerySection` (3 ocurrencias) y `useSecureNavigation` (2). `vite.config.ts` los elimina en build pero contaminan dev | INVENTORY §16.16 | GallerySection: si DESCARTAR en Fase 5.5, se cierra solo. useSecureNavigation: Sprint 2 | Ampliado 2026-05-26. Total auditado en cierre 5.5a: 15 lint errors categorizados. no-explicit-any: `AuthContext.tsx`, `useSecureNavigation.ts`, `dashboard/GallerySection.tsx`, `OrdersSection.tsx`, `PackagesSection.tsx`, `UploadSection.tsx`, varios `src/components/ui/*`. no-empty-object-type: `src/components/ui/*`. no-require-imports: `tailwind.config.ts`. Resolución total Sprint 2. |
| DT-11 | `LegalNotice.tsx` importa `useTranslation` sin usarlo | INVENTORY §16.19 | Fase 4 (REFACTOR páginas legales con prose-editorial) | ✅ Resuelto en Fase 4, commit `434897e` (eliminado import huérfano sin tocar copy real preservado por Protección 2 del brief) |
| DT-12 | `react-compare-slider` instalado pero no usado. `Portfolio.tsx` implementa comparación manualmente | INVENTORY §16.18 | Fase 5.5 — uso confirmado en `PortfolioGallery.tsx` (Bloque C de /portfolio) y `HomePhotoShowcase.tsx` (Bloque 4 de Home) | ✅ Cerrado 2026-05-26. Implementado con `react-compare-slider v4` (no `react-compare-image` como decía documentación original; este último está sin mantener desde 2021). Uso confirmado en `HomePhotoShowcase.tsx` (5.1) y `PortfolioGallery.tsx` (5.5a). Selector DOM `[data-rcs="root"]`. Drift documental cerrado en commit docs sync 5.5a. |
...
| DT-15 | Keys i18n huérfanas: `portfolioShowcase.goldencoast.*` (7 keys) y `photoPacks.trial/basic/standard/premium.*` nunca renderizadas | CONTENT.md §9.1, sesión Fase 1 | Fase 5.1 (REBUILD de `es.json` y `en.json`) | ✅ Resuelto en Subfase 5.1, commit `d7578b7` — keys eliminadas de ambos locales, namespace `home.*` añadido completo |
| DT-16 | Token `--cta` activo (no huérfano como se catalogó inicialmente) en `tailwind.config.ts`. Auditoría grep en sub-tarea 2 (Step 5) detectó uso vivo en 15 archivos legacy: About, Hero, Footer, Services, Header, Testimonials, ProjectCard, FormSection, PackagesSection, GallerySection, UploadSection, Auth, NotFound, button.tsx + propio config | Re-clasificado en Fase 3 sub-tarea 2 tras grep (cataloging error en sub-tarea 1) | **Fase 5** (rediseño página por página). Limpieza natural cuando los componentes legacy se reescriban con `--primary` directo. Coherente con D28 (deuda técnica progresiva) | 🟡 Cerrado parcial 2026-05-26. `HorizontalShowcase` y `FloatingElements` ya inexistentes en repo al inicio de 5.5a (drift de auditoría documental). Restos pendientes en `StickyScrollSection`, `WebPortfolioShowcase`, `ProjectCard` que se descartan en 5.5b. |
| DT-17 | Namespace `pricing.*` en `src/i18n/locales/*.json` con 3 keys `pricing.photoPacks.*` asimétricas históricas (presentes en EN, ausentes en ES). Antigravity las añadió en ES en Subfase 5.4 para mantener `diff: 0` del assert de paridad i18n. Namespace es legacy, no renderizado por componentes del rediseño v2 | O2 reporte Subfase 5.4 (commit `5605764`) | Subfase 5.5 — auditar uso real con `git grep "pricing\\." -- src/` antes de tocar locales. Si confirmado huérfano, eliminar namespace completo. Si vive bajo algún componente preservado, conservar | ✅ Cerrado 2026-05-26. Audit pre-brief 5.5a `git grep -n "pricing\\." -- src/` devolvió cero importadores. Namespace `pricing.photoPacks` (2 nodos padre `corporate` y `custom` con sub-hojas) eliminado de `es.json` y `en.json`. Paridad i18n diff:0 confirmada (452 keys cada uno). |
...
| D35 | 2026-04-30 | **Chrome global funcional (cierre Fase 3 sub-tarea 3, G3 final).** Cuatro decisiones de implementación: D35-1 Header sticky con `bg-background/80 backdrop-blur-md`, layout 3 columnas (logo + nav 5 items + LanguageToggle + WhatsAppButton + hamburguesa mobile). D35-2 Footer 4 columnas (branding + Servicios + Recursos + Contacto) con bottom bar copyright + legal links, único icono social Instagram tras decisión owner. D35-3 i18n cableado: LanguageToggle ES/EN con `i18next-browser-languagedetector` (actualización del config previo cuya `getStoredLanguage` siempre devolvía "es"), persistencia localStorage, `<html lang>` actualizado dinámicamente. D35-4 WhatsApp button con `wa.me/{VITE_WHATSAPP_NUMBER}` real (34667326300) y mensaje pre-formateado i18n, fallback null + console.warn DEV si .env no tiene número. Sub-decisión D35-5 documental: legal routes alineadas con realidad del repo (`/privacidad`, `/cookies`, `/aviso-legal`, NO `/legal/*` como decía el brief). Sub-decisión D35-6: doble chrome aceptado en `Index.tsx` y `PortfolioWebs.tsx` (legacy header/footer + nuevo PublicLayout) hasta Fase 5 cuando se reescriban. Sub-decisión D35-7: logo SVG placeholder en `public/logo.svg` (círculo naranja con "SP") hasta tener logo real | Componentes layout encapsulados, reutilizables sin modificación a partir de Fase 4. i18n verdadero (no falsa promesa) operativo desde G3 final. Defensa anti-flash + chrome global protegen UX en producción. Fix de `getStoredLanguage` cierra Q6 sin abrir DT-17. Validación G3 final 20/20 puntos pasados (Header + Footer visuales, navegación, language toggle ES↔EN con `<html lang>` correcto, WhatsApp con número real, Instagram URL real, mobile drawer funcional) | Fase 3 cerrada. Próxima revisión en Fase 4 cuando se construyan rutas reales |
| D36 | 2026-05-03 | **Cierre Fase 4 (G4 firmado 14/14).** Cinco sub-decisiones de scope/ejecución: D36-1 avance limpieza chrome legacy en Index.tsx y PortfolioWebs.tsx (supersede D35-6, ejecuta Q10=A en mismo paso que reescritura cuerpo). D36-2 modelo definitivo anchors-in-Home: rutas /servicios, /sobre, /contacto descartadas oficialmente, viven como anchors /#services, /#about, /#contact (supersede entries históricos de §5.3). D36-3 placeholder strategy lorem ipsum hard-coded sin keys i18n para Fase 4, keys reales en Fase 5 con copy de CONTENT.md. D36-4 extensión ScrollToTop.tsx para escuchar cambios de hash y disparar scrollIntoView (necesario para que anchors funcionen, decisión correcta de Antigravity al ser componente shared no blindado, packaging mejorable: bundleado con commit de tests en 4d42340). D36-5 copy legal real preservado (Protección 2 disparada correctamente), DT-11 cerrado al paso eliminando import useTranslation huérfano | Fase 4 ejecutada limpiamente: 8 commits (b38e56a a 4d42340), build limpio, tsc limpio, 6/6 tests Playwright pasan, 14/14 puntos validación owner. Lección operativa D36-4 registrada para futuros briefs: listar componentes shared (ScrollToTop, PageLoader, SectionDivider) explícitamente en sección de scope o blindaje | No |
| D37 | 2026-05-08 | **Cierre Subfase 5.1 — Home REBUILD (11 commits, d7578b7 como último i18n).** Cinco sub-decisiones de arquitectura y ejecución: D37-1 estructura de carpeta `src/components/home/` con 7 componentes nuevos (Hero, HomeServices, HomeCasesWeb, HomePhotoShowcase, HomeProcess, HomeAbout, Testimonials). D37-2 HomeCasesWeb usa scroll horizontal CSS nativo (`overflow-x-auto snap-x snap-mandatory`) sin GSAP — coherente con restricción "CSS-only motion en Fase 5" del brief; spacer divs laterales alinean carrusel con el container. D37-3 HomePhotoShowcase solo muestra `galleryData[0]` como teaser (no recorre el array completo); la galería completa es responsabilidad de Fase 5.5 `/portfolio`. D37-4 `webCasesData.ts` define `tagKeys: string[]` con claves i18n (no strings literales), alineado con patrón i18n del proyecto. D37-5 avance de `ProjectCard.tsx` a DESCARTAR en Step 3 (antes de lo previsto en MIGRATION.md §5): era único consumidor de `showcaseData.ts` que también se descartaba, adelantarlo evita import roto sin consumidores | Subfase 5.1 ejecutada limpiamente: 11 commits, build limpio, tsc limpio, grep `--cta` vacío en todos los home components e Index.tsx, 14 archivos legacy eliminados. Lección operativa D37-5: cadenas de imports entre archivos-a-eliminar deben verificarse antes de ordenar eliminaciones por fase en MIGRATION.md | No |
| D38 | 2026-05-12 | **Cierre Subfase 5.2 — Chrome global (5 commits, adb524b a da9662c).** Tres sub-decisiones: D38-1 formalización `src/components/layout/` como ubicación canónica del chrome (supersede prescripción "estructura plana" de §5.2 versiones anteriores, análogo a D36-2). D38-2 REFACTOR ascendido a REBUILD selectivo en `layout/Footer.tsx` por namespace `footer.col.*` legacy desalineado de CONTENT.md §6.2 (decisión técnica correcta de Antigravity, commit `6dca42c`). D38-3 drift `footer.social.instagramUrl` en namespace legacy `footer.*` preservado, limpieza diferida a Subfase 5.4 o 5.5. Corrección documental DT-01: conteo actualizado a 5/7 (Portfolio.tsx limpio por grep). DT-09 marcado resuelto retroactivo (cerrado en Fase 3 D35-3, no documentado en §7.4). CONTENT.md §6.1 actualizado con `common.nav.about` (5.º nav item real omitido en v1.x). | Subfase 5.2 ejecutada: 5 commits, build limpio, tsc limpio, Playwright 9/9, G5 parcial 5.2 firmado 6/6. Lección operativa: namespace i18n del chrome Fase 3 no era canónico internamente (footer.col.* vs common.footer.*), REFACTOR en Footer ascendió a REBUILD selectivo. | No |
| D39 | 2026-05-13 | **Cierre Subfase 5.4 — Contact form REBUILD (3 commits, 92f7cf2 a 3450fb5).** Nueve sub-decisiones: D39-1 estructura `src/components/home/HomeContact.tsx` adoptada como canónica, supersede MIGRATION.md §11 paso 1 ("FormSection.tsx REBUILD") al revelar inspección previa (Q5-B=B) que FormSection era wrapper orphan no renderizado y ContactForm era el componente con lógica. Análogo a D37-1 y D38-1 en filosofía de subcarpetas `src/components/<group>/`. D39-2 ContactForm.tsx y FormSection.tsx ambos DESCARTADOS como código orphan (placeholder lorem D36-3 los reemplazaba en Index.tsx, cero importadores activos confirmado por grep). D39-3 campo `service` preservado con dropdown 3 opciones (web/photo/both) según Q5-E=A, 5 keys i18n nuevas añadidas a `home.contact.form.serviceLabel/servicePlaceholder/serviceOptions.*`. D39-4 checkbox `terms` con link a `/privacidad` preservado por requisito RGPD según Q5-F=A, 2 keys i18n nuevas `home.contact.form.termsLabel/termsLink`. D39-5 mensaje WhatsApp compuesto en texto plano formato `campo: valor` sin emojis ni asteriscos según Q5-G=C, coherente con CONTENT §0 principio editorial. Namespace nuevo `home.contact.whatsapp.*` con 6 keys (subject + 5 labels). D39-6 DOMPurify integrado directo en HomeContact (no via wrapper de security.ts) tras descubrimiento de que los wrappers `sanitizeInput` y `sanitizeHtml` existentes no cubren el caso "strip total a texto plano" requerido por el payload WhatsApp. Corrige drift documental MIGRATION.md §3 que afirmaba que ContactForm legacy ya usaba DOMPurify, cuando nunca lo hizo. D39-7 DT-01 cerrado 7/7 al descartar los 2 últimos archivos con hardcodes (`whatsappNumber` en ContactForm L60 + `tel:+34667326300` en FormSection L47). Conteo retroactivo: en realidad eran 2 hardcodes en 2 archivos, no 1 hardcode pendiente como sugería el contador "5/7" de cierre 5.2. D39-8 lección operativa shadcn FormMessage + i18n: el patrón inicial `<FormMessage>{translateError(...)}</FormMessage>` es incompatible con la implementación actual de shadcn (`FormMessage` ignora children cuando hay error, prioriza `error.message`). Solución adoptada: schema Zod movido dentro del componente con `useMemo([t])`, mensajes ya traducidos en la definición Zod, `<FormMessage />` sin children. Documentar para futuros forms del proyecto. D39-9 patrón Playwright `locale: 'es-ES'` en context obligatorio para tests del rediseño (O3 reporte): sin esto, headless Chromium reporta `en-US` y `i18next-browser-languagedetector` carga EN, lo que rompe selectores `get_by_label` que asumen ES. MASTER §9.4 actualizado con ejemplo canónico. Observaciones no bloqueantes registradas: O1 wa.me redirige a api.whatsapp.com/send en headless (aserción de test ampliada para aceptar ambas formas), O2 3 keys `pricing.photoPacks.*` asimétricas históricas rellenadas en ES para cumplir paridad `diff: 0` (abre DT-17 para auditar en 5.5), O3 ya cubierta por D39-9. | Subfase 5.4 ejecutada: 3 commits (no 4 como brief preveía, agrupación justificada D1 staging area), build limpio, tsc limpio, Playwright 4/4 PASS, G5 parcial 5.4 firmado 14/14. DT-01 cerrado total. DT-17 abierto. Lección operativa D39-8 registrada para futuros forms. | No |
| D40 | 2026-05-26 | **Cierre Subfase 5.5a — /portfolio REBUILD (1 commit, `f49ceac`).** Seis sub-decisiones: D40-1 grid de galería 2 cols en `md:` con último ítem (posición 9) full-width `md:col-span-2`, mobile 1 col (Q5.5-A=A). Decisión editorial: rompe la regularidad del grid y da peso de cierre a la última imagen, alineado con dirección Editorial Structural (MASTER §6.1 prohíbe grids 2x2 uniformes en Home, espíritu trasladado a portfolio). D40-2 split de Fase 5.5 en 5.5a (/portfolio) y 5.5b (/portfolio-webs) por dependencia de componentes reutilizables `PortfolioHeader` y `PortfolioClosing` y para validar primera integración real de slider antes/después en aislamiento antes de aplicar a /portfolio-webs (Q5.5-D=B). D40-3 layout `PortfolioWebsCases` apilado vertical full-bleed (Q5.5-C=A): cada caso hero full-bleed edge-to-edge 16:9 seguido de bloque texto contenido `max-w-2xl` con eyebrow + H3 Playfair + body + tags, separación generosa `py-24 md:py-32` entre casos. D32 satisfecho con confirmación en chat 2026-05-26 (aplica solo en 5.5b). D40-4 chrome de páginas `Portfolio.tsx` y `PortfolioWebs.tsx` añadido desde `@/components/layout/*` (no swap porque audit pre-brief confirmó que legacy no importaba Header/Footer, solo Heading + Button + Link, Q5.5-E resuelto sin swap). D40-5 DT-17 cierre autónomo confirmado en audit pre-brief (cero importadores en src/): eliminación completa del namespace `pricing.photoPacks` (2 nodos padre `corporate` y `custom` con sub-hojas) de `es.json` y `en.json`. Paridad i18n diff:0 confirmada con 452 keys cada uno. D40-6 librería slider real es `react-compare-slider v4` (no `react-compare-image` como decía documentación canónica anterior); decisión técnica de Antigravity correcta dado que `react-compare-image` está sin mantener desde 2021 y `react-compare-slider` ofrece API moderna con selector DOM `[data-rcs="root"]`. Drift documental cerrado en docs sync con find+replace en MASTER §3.8/§6.3, CONTENT §3.4/§4.3, MIGRATION §4/§6. Observaciones no bloqueantes registradas: O-1 librería slider drift (cubierta por D40-6), O-2 conteo de keys eliminadas en DT-17 (2 nodos padre con sub-hojas, no 3 keys planas como decía el brief, resultado equivalente), O-3 `HorizontalShowcase.tsx` y `FloatingElements.tsx` ya inexistentes en repo al inicio (drift de auditoría documental, sin acción inmediata). Desviación D-1 lint pre-existente justificada (15 errores en archivos protegidos no atribuibles al PR, ampliados en DT-10). | Subfase 5.5a ejecutada: 1 commit limpio, build 13.27s, tsc limpio, Playwright `tests/portfolio_smoke.py` 6/6 PASS con locale `es-ES`, G5 parcial 5.5a firmado. DT-12 cerrado, DT-17 cerrado, DT-16 parcial. Patrón `PortfolioHeader`/`PortfolioClosing` reutilizables validado para 5.5b. Lecciones operativas adicionales: (a) docs sync con find+replace masivo en PowerShell 5.x requiere método encoding-safe (`[System.IO.File]::WriteAllText` con `UTF8Encoding $false`, no `Set-Content -Encoding UTF8` que escribe BOM y corrompe UTF-8 sin BOM preexistente); (b) VS Code debe configurarse explícitamente con `files.encoding = utf8` y `files.autoGuessEncoding = false` para evitar re-codificación al guardar. | No |
...
| 1.7 | 2026-04-30 | Miguel + Claude Opus 4.7 | Cierre Fase 3 completa (G3 final firmado). §13 nueva entrada D35 con sub-decisiones D35-1 a D35-7: chrome global funcional (Header + Footer + i18n + WhatsApp real + ajustes documentales sobre legal routes, doble chrome temporal, logo placeholder). Sin cambios en §3.x (DS) ni §7.x (deuda técnica) en este cierre: la sub-tarea 3 fue puramente integración funcional sobre tokens y componentes ya validados en sub-tareas 1 y 2. |
| 1.8 | 2026-05-03 | Miguel + Claude Opus 4.7 | Cierre Fase 4 (G4 firmado 14/14). §5.3 reescrita: 10 rutas confirmadas, modelo anchors-in-Home oficializado, /servicios+/sobre+/contacto descartadas como rutas dedicadas, legal routes alineadas con realidad del repo. §7.4 DT-11 cerrado con commit `434897e`. §13 D36 con sub-decisiones D36-1 a D36-5: avance limpieza chrome legacy, modelo anchors-in-Home, placeholder strategy lorem hard-coded, extensión ScrollToTop para hash navigation (lección operativa registrada), copy legal real preservado por Protección 2. |
| 1.9 | 2026-05-08 | Miguel + Claude Sonnet 4.6 | Cierre Subfase 5.1 Home REBUILD. §0 versión bumped. §7.4 DT-15 marcado resuelto (commit `d7578b7`). §13 nueva entrada D37 con sub-decisiones D37-1 a D37-5: arquitectura carpeta home/, scroll CSS nativo en HomeCasesWeb, teaser galleryData[0] en HomePhotoShowcase, tagKeys i18n en webCasesData, avance eliminación ProjectCard. |
| 2.0 | 2026-05-12 | Miguel + Claude Sonnet 4.6 | Cierre Subfase 5.2 Chrome global. §0 versión bumped. §5.2 árbol de carpetas actualizado con `home/` y `layout/` + nota D38-1. §7.4 DT-01 campo Estado corregido a 5/7. §7.4 DT-09 marcado resuelto con nota de drift documental. §13 entrada D38 con sub-decisiones D38-1 a D38-3. |
| 2.1 | 2026-05-13 | Miguel + Claude Opus 4.7 | Cierre Subfase 5.4 Contact form REBUILD. §0 versión bumped. §7.4 DT-01 marcado ✅ 7/7 con 3 commits de la subfase. §7.4 DT-02 actualizado con cierre parcial Sprint 1 (ContactForm y processData descartados). §7.4 nueva entrada DT-17 (namespace `pricing.*` legacy con 3 keys ES añadidas para paridad, auditar en 5.5). §9.4 patrón base Playwright ampliado con `locale: 'es-ES'` obligatorio en context (lección O3). §13 entrada D39 con sub-decisiones D39-1 a D39-9 (HomeContact canónico, descarte legacy, service preservado, terms preservado RGPD, mensaje texto plano, DOMPurify directo, DT-01 total, lección FormMessage+i18n, lección locale Playwright). |